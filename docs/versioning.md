...
sending a response to a request. S3 also decodes the hexadecimal string
...
engine can handle. The process for creating new versions or delete markers has
additional steps and is described more comprehensively in the section
"Implementation of Bucket Versioning in API" and its subsections.
...
- `nullVersionId`: the version ID of the latest null version.
- `isDeleteMarker`: whether the version being stored is a delete marker.
- `versionId`: the version ID of the version being stored.
...
should exist at any given time, and it can be identified in S3 requests
and responses with the version id "null".
...
as opposed to being stored in the master key and a new version, like how normal
versions are stored. The metadata engine does not differentiate between null and
normal versions, so S3 checks if versioning is suspended or has never been
configured, and sets the `versionId` option to `''` in PUT calls to the metadata
engine.
...
re-enabled, and new non-null versions are created. To preserve the null version
if the null version is the master version, the the S3 API stores the current
null version as a new key `(3A)` prior to overwriting the master version `(3B)`.
This implies the null version may not necessarily be the latest or master
version.

To continue the example immediately above, if we enabled versioning and
put another object, the calls to metadata and resulting keys would resemble the
following:
...
Now, in the case versioning is suspended and the null version is not the latest
...
(4A) DELETE foo, versionId: `v1` (null version ID)
...
Note that S3 keeps track of the internally-used version ID of the current null
version in the `nullVersionId` metadata attribute for each new version.
In addition, to determine whether the null version is the latest version, the S3
API sets the `isNull` attribute for each version. The `nullVersionId` attribute
is also used to retrieve the correct version when the version ID "null" is
specified in certain object-level APIs, described further in the section
...
- If it has not been configured, S3 proceeds to delete any pre-existing data and
  put the new object as the master version in Metadata

If it has been configured, S3 checks the following:

#### Versioning Enabled

If versioning is enabled and there is existing object metadata:

- If the master version is a null version or has no version ID (put before
...
In each of the above cases, set `isNull` metadata attribute to true when when
...
One exception are the Delete Object and Multi-Object Delete APIs, which will
...
the current null version. Thus, getting the null version is a two step process:
