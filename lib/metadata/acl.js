...
const acl = {
    ...
    addObjectACL(bucket, objectKey, objectMD, addACLParams, params, log, cb) {
        ...
        // We only replicate objects in the source bucket for which the bucket
        // owner has permission to read objects and read the ACLs.
        const bucketOwner = bucket.getOwner();
        const objectRead = objectMD.acl.READ;
        const objectReadACP = objectMD.acl.READ_ACP;
        const replicationConfig = bucket.getReplicationConfiguration();
        if (replicationConfig &&
            objectRead.includes(bucketOwner) &&
            objectReadACP.includes(bucketOwner)) {
            Object.assign(objectMD.replication, {
                status: 'PENDING',
                content: ['METADATA'],
            });
        }
