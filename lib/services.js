...
const ObjectModelMD =
    require('./metadata/models/object/ObjectModelMD').ObjectModelMD;
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        const { objectKey, authInfo, size, contentMD5, metaHeaders,
            ...
            lastModifiedDate, versioning, versionId, tagging,
            replication } = params;
        log.trace('storing object in metadata');
        assert.strictEqual(typeof bucketName, 'string');
        const md = new ObjectModelMD();
        ...
        if (replication) {
            md.setReplicationInfo(replication);
        }
        ...
        log.trace('object metadata', { omVal: md.getData() });
