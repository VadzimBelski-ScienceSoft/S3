...
const { validateUserMetadata } = require('./api/apiUtils/object/storeObjectMd');
...
const services = {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        const validateUserMdRes = validateUserMetadata(metaHeaders);
        if (validateUserMdRes instanceof Error) {
            log.debug('user metadata validation failed', {
                error: validateUserMdRes,
                method: 'metadataStoreObject',
            });
            return process.nextTick(() => cb(validateUserMdRes));
        }
