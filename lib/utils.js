...
utils.handleReplicationMD = function setReplicationMD(bucketMD, objMD,
    objectKey, objMetadata) {
    const replicationConfig = bucketMD.getReplicationConfiguration();
    if (replicationConfig) {
        for (let i = 0; i < replicationConfig.rules.length; i++) {
            const rule = replicationConfig.rules[i];
            if (objectKey.startsWith(rule.prefix)) {
                ...
                if (rule.storageClass) {
                    replication.storageClass = rule.storageClass;
                }
                objMetadata.replication = replication;
                break;
            }
        }
    }
