...
/**
 * Check if the replication configuration exists and contains an applicable rule
 * @param {string} objKey - The key of the object
 * @param {object} [config] - The bucket replication configuration
 * @return {object|undefined} The replication rule if found, otherwise
 * `undefined`
 */
utils.getReplicationRule = function getReplicationRule(objKey, config) {
    if (config) {
        return config.rules.find(rule => objKey.startsWith(rule.prefix));
    }
    return undefined;
};

/**
 * Get the object metadata replication information to set
 * @param {string} config - The bucket replication configuration
 * @param {object} rule - The replication rule
 * @param {array} content - The array of content types to be replicated
 * @return {object} The object metadata replication information
 */
utils.getReplicationInfo = function getReplicationInfo(config, rule, content) {
    const replicationInfo = {
        status: 'PENDING',
        content,
        destination: config.destination,
    };
    // StorageClass is optional for rules.
    if (rule.storageClass) {
        Object.assign(replicationInfo, { storageClass: rule.storageClass });
    }
    return replicationInfo;
};

/**
 * Set the object's metatdata to indicate that the object's data and metadata
 * should be replicated. If object is delete marker, replicate only metadata.
 * @param {object} bucketMD - The bucket metadata
 * @param {string} objKey - The key of the object
 * @param {object} objMD - The object metadata
 * @return {undefined}
 */
utils.replicateObject = function replicateObject(bucketMD, objKey, objMD) {
    const config = bucketMD.getReplicationConfiguration();
    const rule = this.getReplicationRule(objKey, config);
    if (rule) {
        // Delete markers have no data, so we only replicate metadata.
        const content = objMD.isDeleteMarker ? ['METADATA'] :
            ['DATA', 'METADATA'];
        objMD.replicationInfo = this.getReplicationInfo(config, rule, content);
    }
    return undefined;
};

/**
 * Set the object's metatdata to indicate that the object's metadata should be
 * replicated
 * @param {object} bucketMD - The bucket metadata
 * @param {string} objKey - The key of the object
 * @param {object} objMD - The object metadata
 * @return {undefined}
 */
utils.replicateObjectMD = function replicateObjectMD(bucketMD, objKey, objMD) {
    const config = bucketMD.getReplicationConfiguration();
    const rule = this.getReplicationRule(objKey, config);
    if (rule) {
