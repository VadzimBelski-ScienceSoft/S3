...
function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        // If data backend is real AWS, no need to store part metadata
        (destinationBucket, objectLocationConstraint, cipherBundle,
        splitter, next) => {
            if (config.backends.data === 'multiple') {
                return multipleBackendGateway.uploadPart(
                destinationBucket, objectLocationConstraint, objectKey,
                uploadId, partNumber, log, (err, data) => {
                    if (err) {
        ...
        if (err) {
            if (err === skipError) {
                return cb(null, null, corsHeaders);
            }
