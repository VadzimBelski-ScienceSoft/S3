...
const validCloudBackends = { 'aws-test': true };
    ...
    return async.waterfall([
        ...
        function getMPUMetadata(destBucket, objMD, mpuBucket, jsonList, next) {
            ...
            {}, log, (err, storedMetadata) => {
                ...
                storedMetadata, location);
            });
        },
        function ifMultipleBackend(destBucket, objMD, mpuBucket, jsonList,
        storedMetadata, location, next) {
            const isCloudBackend = validCloudBackends[location] || false;
            if (isCloudBackend) {
                return multipleBackendGateway.completeMPU(objectKey,
                uploadId, location, jsonList, log, (err, newObjData) => {
                    if (err) {
                        return next(err, destBucket);
                    }
                    return next(null, destBucket, objMD, mpuBucket,
                    jsonList, storedMetadata, newObjData);
                });
            }
            return next(null, destBucket, objMD, mpuBucket, jsonList,
            storedMetadata, null);
        ...
        storedMetadata, newObjData, next) {
        ...
        function processParts(destBucket, objMD, mpuBucket, storedParts,
            jsonList, storedMetadata, newObjData, next) {
            let calculatedSize = 0;
            if (newObjData) {
                return next(null, destBucket, objMD, mpuBucket,
                    storedMetadata, newObjData.ETag, calculatedSize,
                    null, mpuOverviewKey, null, null, newObjData);
            }
        ...
        function prepForStoring(destBucket, objMD, mpuBucket, storedMetadata,
            aggregateETag, calculatedSize, dataLocations, mpuOverviewKey,
            storedPartsAsObjects, extraPartLocations, newObjData, next) {
            const metaHeaders = {};
            const keysNotNeeded =
        ...
        function storeAsNewObj(destinationBucket, dataLocations,
            ...
            return services.metadataStoreObject(destinationBucket.getName(),
                ...
                (err, res) => {
                    ...
                    if (newObjData) {
                        generatedVersionId = newObjData.VersionId || undefined;
                    } else {
                        generatedVersionId = res ? res.versionId : undefined;
                    }
                    ...
                    return next(null, mpuBucket, mpuOverviewKey, aggregateETag,
                        ...
                        destinationBucket, generatedVersionId, newObjData);
                });
        },
        function deletePartsMetadata(mpuBucket, mpuOverviewKey, aggregateETag,
            ...
            generatedVersionId, newObjData, next) {
            let keysToDelete = [];
            if (!newObjData) {
