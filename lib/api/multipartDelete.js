...
function multipartDelete(authInfo, request, log, callback) {
    ...
    async.waterfall([
        ...
        function ifMultipleBackend(destBucket, next) {
            const locationConstraint = destBucket ?
                destBucket.getLocationConstraint() : null;
            if (config.backends.data === 'multiple') {
                return multipleBackendGateway.abortMPU(objectKey,
                uploadId, locationConstraint, log, err => {
                    if (err) {
                        return next(err, destBucket, locationConstraint);
                    }
                    return next(null, destBucket, locationConstraint);
                });
            }
            return next(null, destBucket, locationConstraint);
        },
    ], (err, destinationBucket, locationConstraint) => {
